...
function SessionScreen({ sessionId, revision, go, changed, fail, canUndo, undoDelete }: SharedProps & { sessionId: Uuid; revision: number; canUndo: boolean; undoDelete: () => Promise<void> }) {
  const [session, setSession] = useState<FieldSession | null>(null); const [observations, setObservations] = useState<Observation[]>([]); const [assets, setAssets] = useState<Asset[]>([]); const [mediaCounts, setMediaCounts] = useState<Record<string, number>>({});
  ...
  const load = useCallback(async () => {
    ...
    const media = await Promise.all(nextObservations.map(async (observation) => [observation.id, (await repositories.listMedia(observation.id)).length] as const));
...
function CaptureScreen({ sessionId, go, changed, fail }: SharedProps & { sessionId: Uuid }) {
  ...
  const [category, setCategory] = useState<ObservationCategory | null>(null); const [value, setValue] = useState<string | null>(null); const [evidence, setEvidence] = useState<EvidenceForm>(emptyEvidence); const [note, setNote] = useState(''); const [photo, setPhoto] = useState<File | null>(null);
  ...
  const save = async () => {
    ...
    try {
      ...
      let message = 'Observation saved locally.';
      if (photo) { try { await repositories.addMedia({ observationId: observation.id, kind: 'photo', blob: photo, mimeType: photo.type || 'application/octet-stream', originalFilename: photo.name || null }); message = 'Observation and photo saved locally.'; } catch { message = 'Observation saved, but the photo could not be stored. Back up or free device space.'; } }
