 ...
 *   - Dexie DB version 3  ← physical stores/indexes (P1-5 added `observationAudit`; Campaign +
 *                            FieldPack v1 added the `campaigns` store and `campaignId` indexes)
 *   - FieldOS schema version 5  ← logical/canonical shape
 ...
 *   - 5  Campaign + FieldPack v1: a new `FieldCampaign` entity (its own `campaigns` store), plus an
 *        additive nullable `campaignId` on FieldSession, and additive nullable `campaignId` +
 *        `sourceRef` on Asset. The new store AND the new `campaignId` indexes on existing stores
 *        require a real Dexie upgrade (DB version 2 → 3); the additive nullable fields on existing
 *        rows normalize to `null` at the read boundary without rewriting legacy rows.
 */
